...
function getApiAccount({ region, account }) {
  // Recuerso de la API al que queremos acceder
  const resource = `d3/profile/${account}/`
  // API URL completa
  const API_URL = `${protocol}${region.toLowerCase()}${host}${resource}`
  // Locale
  ...
  // Retornaremos el resultado de hacer la llamada a la API, es decir, una promesa
  // que controlaremos (exito / error) desde el componente
